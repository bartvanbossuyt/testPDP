        ...
        with _6evs_pcol2b:
            _6evs_buffer_y = st.number_input(
                ...
                help="Tolerance on lateral (cross-lane) ordering. "
                     "Points within this distance are considered equivalent in d2. "
                     "Only used by buffer/bufferrough variants.",
            )
        with _6evs_pcol3:
    ...
    for rank, (config_num, deviation, config) in enumerate(top_100, 1):
            ...
            if not _is_custom_upload:
                ...
                for ax in (ax_left, ax_right):
                    ...
                    ax.axhspan(_lane1_c - lane_width / 2, _lane1_c + lane_width / 2,
                               color='#A9A9A9', alpha=0.35, zorder=0)
                    ax.axhspan(_lane2_c - lane_width / 2, _lane2_c + lane_width / 2,
                               color='#A9A9A9', alpha=0.35, zorder=0)
                    ...
                    ax.axhline(_divider_y, color='white', linewidth=1.2, linestyle='--', zorder=1)
...
Configurations are ranked by perpendicular variance (highest first). Each visualization shows the complete generated trajectory with lane markings for context.""")
    ...
    for rank, (config_num, deviation, config) in enumerate(top_500, 1):
            ...
            if not _is_custom_upload:
                ...
                _lane2_c = avg_y - lane_width / 2.0
                # Filled gray lane surfaces (6-event style)
                ax_right.axhspan(_lane1_c - lane_width / 2, _lane1_c + lane_width / 2,
                                 color='#A9A9A9', alpha=0.35, zorder=0)
                ax_right.axhspan(_lane2_c - lane_width / 2, _lane2_c + lane_width / 2,
                                 color='#A9A9A9', alpha=0.35, zorder=0)
                # Solid black outer road edges
                _road_top = _lane1_c + lane_width / 2
                _road_bot = _lane2_c - lane_width / 2
                ...
                # Dashed white center line between lanes
                _divider_y = (_lane1_c + _lane2_c) / 2.0
                ax_right.axhline(_divider_y, color='white', linewidth=1.2, linestyle='--', zorder=1)
...
if st.session_state.get("_generate_6ev_single_results", None):
    ...
    if _6evs_browse_idx == 0:
        ...
        if _c0_timestamps:
            ...
            _c0_fig.update_layout(
                ...
                xaxis_title="d1 / x-as (m)", yaxis_title="d2 / y-as (m)",
        ...
        if _c0_timestamps:
            ...
            if _6evs_show_lanes_val:
                ...
                _c0_l2c = st.session_state.get("_6evs_lane2_center", -3.0)
                _c0_lw = 3.0
                for _cy, _ln in [(_c0_l1c, "Lane 1"), (_c0_l2c, "Lane 2")]:
                    _c0_sfig.add_hrect(
                        y0=_cy - _c0_lw / 2, y1=_cy + _c0_lw / 2,
    ...
    if _smooth_btn and _6evs_plot_data:
        ...
        for _fr_idx, _t_cur in enumerate(_smooth_t_values):
            ...
            # ---- Draw external reference points (gray squares) ----
            _anim_ext_pts = st.session_state.get("external_points", [])
            if _anim_ext_pts:
                _anim_ext_coords = []
    ...
    # Build a CSV with the generated 6-event timestamp configuration
    _6evs_csv_rows: list[dict[str, object]] = []
    _6evs_ev_labels_csv = {0.0: "E1 start", 34.0: "E2 lane-change", 67.0: "E3 arrived",
                           183.0: "E4 return", 213.0: "E5 arrived", 249.0: "E6 end"}
    for idx_o, oid in enumerate(_6evs_sorted_oids):
        orig, gen, ts = _6evs_plot_data[idx_o]
        lbl = OBJECT_LABELS[int(oid) % len(OBJECT_LABELS)]
        for li in range(orig.shape[0]):
            _t_val = float(ts[li])
            _ev_name = _6evs_ev_labels_csv.get(_t_val, f"t={int(_t_val)}")
            _6evs_csv_rows.append({
                "object": lbl,
                "event": _ev_name,
                "t": int(ts[li]),
                "orig_d1": round(float(orig[li, 0]), COORD_CSV_PRECISION),
                "orig_d2": round(float(orig[li, 1]), COORD_CSV_PRECISION),
                "gen_d1": round(float(gen[li, 0]), COORD_CSV_PRECISION),
                "gen_d2": round(float(gen[li, 1]), COORD_CSV_PRECISION),
                "delta_d1": round(float(gen[li, 0] - orig[li, 0]), COORD_CSV_PRECISION),
                "delta_d2": round(float(gen[li, 1] - orig[li, 1]), COORD_CSV_PRECISION),
            })
    # Include external (fixed) reference points if present
    _6evs_ext_csv = st.session_state.get("external_points", [])
    if _6evs_ext_csv:
        for _ep_idx, _ep in enumerate(_6evs_ext_csv):
            ...
            _6evs_csv_rows.append({
                ...
                "event": "fixed",
                "t": "",
                "orig_d1": round(_ep_x, COORD_CSV_PRECISION),
                "orig_d2": round(_ep_y, COORD_CSV_PRECISION),
                "gen_d1": round(_ep_x, COORD_CSV_PRECISION),
                "gen_d2": round(_ep_y, COORD_CSV_PRECISION),
                "delta_d1": 0.0,
                "delta_d2": 0.0,
            })
    if _6evs_csv_rows:
    ...
    if _6evs_orig_flat.shape[0] > 0:
        ...
        # Build label list matching flat order (obj-first: k₀ k₁ k₂ ... l₀ l₁ l₂ ...)
