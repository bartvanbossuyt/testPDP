...
def inequalities(array_of_1_configuration):
    ...
    # Save dataframe "Df_con_tst_xineq_yineq" into results dir if set
    results_dir = os.environ.get('AV_RESULTS_DIR', os.getcwd())
    os.makedirs(results_dir, exist_ok=True)
    inverse_dir = os.path.join(results_dir, 'Inverse')
    os.makedirs(inverse_dir, exist_ok=True)
    Df_con_tst_xineq_yineq.to_csv(os.path.join(inverse_dir, "Df_con_tst_xineq_yineq.csv"), index=False)
...
for configuration in range(av.num_similar_configurations*av.new_configuration_step):
    ...
    while True:
        ...
        inverse_dir = os.path.join(results_dir, 'Inverse')
        os.makedirs(inverse_dir, exist_ok=True)
        N_PDP.Df_con_tst_xineq_yineq.to_csv(os.path.join(inverse_dir, "basic_inequalities_representation.csv"), index=False)
        result.to_csv(os.path.join(inverse_dir, "new_inequalities_representation.csv"), index=False)
        ...
        if N_PDP.Df_con_tst_xineq_yineq.equals(Df_new_xineq_yineq): #if the new generated configuration is similar to the basic configuration
            ...
            if configuration == 0:
                ...
                destination_file_path = os.path.join(inverse_dir, 'N_C_similar_configurations.csv')
                # Copy the file
            ...
            #the following line has to be done only after the step length
                df_dataset.to_csv(os.path.join(inverse_dir, "N_C_similar_configurations.csv"), index=False)
