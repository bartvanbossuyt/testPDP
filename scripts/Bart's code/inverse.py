...
if st.session_state.get("anim_successful_points"):
    ...
    for sp in successful_points_export:
        point = sp["point"]
        original_idx = sp.get("original_parent_idx", 0)
        
        # Determine if it's k or l based on original_parent_idx
        n_k = k_points_plot.shape[0]
        if original_idx < n_k:
            # k-point (o=0)
            t_val = k_vals_plot[original_idx]
            o_val = 0
        else:
            # l-point (o=1)
            local_idx = original_idx - n_k
            t_val = l_vals_plot[local_idx]
            o_val = 1
        
        # Format: c,t,o,x,y
        csv_lines.append(f"{selected_c_int},{t_val},{o_val},{point[0]:.6f},{point[1]:.6f}")
