...
def _extract_order_string(latex_str: str) -> str:
    ...
    # remove {â€¦} but keep inside, so k_{0} â†’ k_0
...
def check_pdp_match(original_points: np.ndarray, generated_points: np.ndarray,
    ...
    - fundamental: Basic PDP matching with no tolerance (NÃ—N matrix comparison)
    ...
    For buffer variants:
    - Each point is expanded to 5 buffer variants (Â±buffer_x, Â±buffer_y, original)
    - Both original and generated configurations are expanded
    - The resulting 5NÃ—5N inequality matrices are compared
...
def check_pdp_match_detailed(original_points: np.ndarray, generated_points: np.ndarray,
    ...
    Returns:
        ...
        - original_d1_matrix: NÃ—N inequality matrix for original d1
        - original_d2_matrix: NÃ—N inequality matrix for original d2
        - generated_d1_matrix: NÃ—N inequality matrix for generated d1
        - generated_d2_matrix: NÃ—N inequality matrix for generated d2
    """
    # Apply buffer transformation if needed (expands N points to 5N points)
    orig_pts = original_points.copy()
    gen_pts = generated_points.copy()
