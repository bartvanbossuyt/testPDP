...
# Try to find the dataset file (it might be in outputs/D, outputs/Moving_Objects or root directory)
dataset_path = os.path.join(os.getcwd(), dataset_name)  # First try root directory
if not os.path.exists(dataset_path):
    # Try outputs/D folder
    outputs_d_path = os.path.join(os.getcwd(), 'outputs', 'D', dataset_name)
    if os.path.exists(outputs_d_path):
        dataset_path = outputs_d_path
    else:
        # Try outputs/Moving_Objects folder
        outputs_mo_path = os.path.join(os.getcwd(), 'outputs', 'Moving_Objects', dataset_name)
        if os.path.exists(outputs_mo_path):
            dataset_path = outputs_mo_path
...
def get_output_dir(module_name):
    ...
    For example, N_PDP.py will use the 'PDP' folder by removing 'N_' prefix and '.py' extension.
    ...
    """
    outputs_base_dir = os.path.join(os.getcwd(), 'outputs')
    output_dir = os.path.join(outputs_base_dir, module_name)
...
def get_input_file(filename, module_names=None):
    ...
    Searches multiple module directories in order.
    ...
    # Try to find the file in the specified module directories
    for module_name in module_names:
        file_path = os.path.join(outputs_base_dir, module_name, filename)
    ...
    if module_names:
        return os.path.join(outputs_base_dir, module_names[0], filename)
    else:
        return os.path.join(outputs_base_dir, filename)
...
def get_output_file(filename, module_name):
    ...
    # Return the expected path in outputs even if file doesn't exist yet
    return file_path
