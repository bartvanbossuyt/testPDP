...
for con_id in range(av.con):  # Loop over all configurations con_id
    ...
    for tst_id in range(av.tst-(av.window_length_tst-1)):  # Loop over all time stamps, dependant of the window length
	    ...
	    # Df_tst_id = Df_tst_id.sort_values(by=[dim_id]).reset_index(drop=True)
        ...
        for dim_id in ['x', 'y']:  # Loop over all dimensions (x and y)            
            ...
            if av.N_VA_InequalityMatrices == 1:
                ...
                num_obj = int(av.poi)
                # 得到原始顺序labels（ticks实际就是）
                labels = ticks

                # 新顺序：先p0所有时间，再p1所有时间（适用于poi=2, 你也可扩展到任意poi）
                reordered_labels = []
                ...
                plt.figure(figsize=(11, 8), dpi=300.0)  # adjust as needed
                ...
                plt.xticks(range(len(reordered_labels)), reordered_labels, rotation=45, ha='right')  # Rotate labels at 45 degrees and align them to the righ
                plt.yticks(range(len(reordered_labels)), reordered_labels)


                # ------- 新增：添加右上 & 左下象限的白色对角线 -------
                split_idx = num_time  # P0 占据前半；P1 从 split_idx 开始
