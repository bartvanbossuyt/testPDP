...
def Transform_tsne(A_dataset):
    M = av.A_dataset.shape[0]  # Number of points in the matrix
    perp = min(30, M//3)  # 1/3 of data, max 30
    perp = max(perp, 5)  # And not lower than 5
...
for i in range(len(av.A_dataset)):
    ax.annotate(i, xy=(A_embedding[i, 0], A_embedding[i, 1]), xytext=(5, 5), textcoords="offset points")
