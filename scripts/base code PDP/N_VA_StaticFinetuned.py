"""
Visual Analytics: Creates static finetuned visualizations
EXPLANATION
    Creates finetuned static visualizations with customized layout (e.g., for tennis pitch)
INPUT
    Dataset from av module
OUTPUT
    PNG images for each configuration (N_C_Csf*.png)
"""
...
for config in configurations:
    ...
    # Calculate the max of range x and range y
    ...
    if av.tst == 1:
        for point_index, (x_val, y_val) in enumerate(zip(x, y)):
            ...
            plt.scatter(x_val, y_val, color=color_to_use, s=100)
    else:
        vector_index = 0
    ...
    plt.title('Configuration ' + str(config), fontname="Arial", fontsize=40)
